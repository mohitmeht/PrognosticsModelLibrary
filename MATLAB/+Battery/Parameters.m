...
P.sampleTime = 0.09;%0.18;%0.18/2;%1;%0.045; (Prognostics Sample Time)
P.x0.sampleTime = P.sampleTime; % in seconds (Model Simulation Sample Time)
...
P.x0.Vsn = 0;
P.x0.Vsp = 0;
P.x0.Tb = 332.05;            % in K, about 58.9 degC
...
function P = FitChetanforNASAAerospace(qMobile)
P.sampleTime = 0.18;%0.18/2;%1;%0.045; (Prognostics Sample Time)
P.x0.sampleTime = 0.18; % in seconds (Model Simulation Sample Time)
...
P.alpha = 0.5;            % anodic/cathodic electrochemical transfer coefficient
P.Sn = 0.000437545;       % surface area (- electrode)
% P.Sn = P.Sn.*1;           % surface area (- electrode)
P.Sp = 0.00030962;        % surface area (+ electrode)
% P.Sp = P.Sp.*0.5;           % surface area (- electrode)
P.kn = 2120.96;           % lumped constant for BV (- electrode)
P.kp = 248898;            % lumped constant for BV (+ electrode)
P.Vol = 2e-5;             % total interior battery volume/2 (for computing concentrations)
P.VolSFraction = 0.1;     % fraction of total volume occupied by surface volume
...
% time constants
P.tDiffusion = 7e6;  % diffusion time constant (increasing this causes decrease in diffusion rate)
P.to = 8;%6.08671;      % for Ohmic voltage
P.tsn = 200;%1001.38;     % for surface overpotential (neg)
P.tsp = 10;%46.4311;     % for surface overpotential (pos)
...
P.sampleTime = 0.18;%0.18/2;%1;%0.045;
...
P.x0.Vsp = 0;
P.x0.Tb = 332.05;            % in K, about 58.9 degC
P.x0.sampleTime = 0.1; % in seconds
